...
namespace bumperbot_planning {
    AStarPlanner::AStarPlanner() : Node("a_star_node") {
        ...
        rclcpp::QoS map_qos = rclcpp::QoS(10);
    ...
    nav_msgs::msg::Path AStarPlanner::plan(const geometry_msgs::msg::Pose &start, const geometry_msgs::msg::Pose &goal) {
        ...
        while(!pending_nodes.empty() && rclcpp::ok()) {
            // get the first node (node with least cost)
            active_node = pending_nodes.top();

            // remove the first node from pending_node queue
            pending_nodes.pop();
            ...
            // otherwise, explore active node's neighbouring (up, down, left, right)
            for(const auto &direction: explore_directions) {
                ...
                if(std::find(visited_nodes.begin(), visited_nodes.end(), new_node) == visited_nodes.end() &&
                    poseOnMap(new_node) && map_->data.at(poseToCell(new_node)) == 0) {
                        ...
                        // sets all cost to all node to 1
                        new_node.cost = active_node.cost + 1;

                        // initalize heuristic of the new_node
                        new_node.heuristic = manhattanDistance(new_node, goal_node);

                        // initalizes parent node
                        new_node.prev = std::make_shared<GraphNode>(active_node);

                        // adds new discovered neighbor to the pending_nodes
            ...
            // add the visited node to visited list
            visited_map_.data.at(poseToCell(active_node)) = -106; // used for setting color for visualization
